...
void Application::UpdateIdleDisplay() {
    ...
    if (localtime_r(&now, &tm_buf) != nullptr) {
		...
		strftime(buffer, sizeof(buffer), "%d/%m", &tm_buf);
        card.date_text = buffer;
        
        // Thứ tiếng Việt rút gọn
		static const char* WEEKDAY_VI_SHORT[] = {
			"CN", "T2", "T3", "T4", "T5", "T6", "T7"
		};
		card.day_text = WEEKDAY_VI_SHORT[tm_buf.tm_wday];
    }
    ...
    if (weather_info.valid) {
        ...
        char extra_buf[32];
        snprintf(extra_buf, sizeof(extra_buf), "Cảm giác: %d°C", (int)round(weather_info.feels_like));
